...
class ImageGesture(protected val mImageView: ImageView) : AllGesture(), View.OnTouchListener,
    ...
    final override fun onScroll(
        ...
        if (distanceX > 0) {
            if (mDisplayRect.right > viewWidth) handled = true
                ...
                if (abs((e2.x ) - (e1?.x ?: 0f)) - abs((e2.y) - (e1?.y ?: 0f)) > 64) {
        ...
        if (distanceY > 0) {
            if (mDisplayRect.bottom > viewHeight) handled = true
                ...
                if (abs((e2.y ) - (e1?.y ?: 0f)) - abs((e2.x ) - (e1?.x ?: 0f)) > 64) {
                    mImageView.parent?.requestDisallowInterceptTouchEvent(false)
                }
            }
        } else {
